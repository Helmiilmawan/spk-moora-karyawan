...
<div class="container py-2">
    ...
    <div class="mb-4 animate-fade text-center">
        <h3 class="fw-bold text-dark">
            <i class="bi bi-pencil-square text-primary"></i> Edit Alternatif
        </h3>
        <p class="text-muted">Perbarui data alternatif destinasi wisata.</p>
    </div>

    {{-- KARTU PUTIH CLEAN ELEGAN --}}
    <div class="card shadow-lg border-0 rounded-4 glass-card animate-up mx-auto" style="max-width: 900px; background: #ffffff;">
        <div class="card-body p-4">

            <form action="{{ route('alternatives.update', $alternative) }}" 
                  method="POST" 
                ...
                <div class="row">
                    <div class="col-md-6 mb-3">
                        ...
                        <input type="text" class="form-control form-control-lg rounded-3 bg-light text-muted" value="{{ $alternative->code }}" readonly>
                    </div>

                    <div class="col-md-6 mb-3">
                        <label class="form-label fw-semibold text-dark">
                            <i class="bi bi-map-fill text-success"></i> Nama Alternatif
                        </label>
                ...
                <div class="mb-3">
                    <label class="form-label fw-semibold text-dark">
                        <i class="bi bi-image-fill text-warning"></i> Gambar Saat Ini
                    </label>
                    <div class="mb-3 text-center">
                        @if($alternative->image && \Illuminate\Support\Facades\Storage::disk('public')->exists('alternatives/'.$alternative->image))
                            <img src="{{ asset('storage/alternatives/'.$alternative->image) }}"
                                 ...
                                 style="width: 180px; height: 130px; object-fit: cover; border: 1px solid #dee2e6;">
                        @else
                            <p class="text-muted small id-italic">Tidak ada gambar yang tersimpan.</p>
                        @endif
                    </div>

                    <label class="form-label fw-semibold text-dark mt-2">
                        <i class="bi bi-upload text-primary"></i> Upload Gambar Baru (Opsional)
                    </label>
                ...
                <div class="mb-3">
                    ...
                    <textarea name="description" rows="4" 
                              ...
                              placeholder="Tuliskan deskripsi alternatif...">{{ old('description', $alternative->description) }}</textarea>
                    @error('description')
                        <div class="invalid-feedback text-danger">{{ $message }}</div>
                    @enderror
                </div>
                ...
                <div class="mb-4">
                    ...
                    <p class="text-muted text-center small mb-4">
                        Perbarui nilai kriteria kandidat yang akan digunakan dalam proses perhitungan MOORA.
                    </p>

                    <div class="row">
                        @foreach($criteria as $c)
                        @php
                            $currentRating = $alternative->ratings->where('criterion_id', $c->id)->first();
                        @endphp
                        ...
                        <div class="col-md-6 mb-3">
                            ...
                            <select name="criteria[{{ $c->id }}]" class="form-select rounded-3" required>
                                ...
                                @foreach($c->subCriteria->sortByDesc('value') as $sub)
                                    <option value="{{ $sub->value }}" {{ $currentRating && $currentRating->value == $sub->value ? 'selected' : '' }}>
                                        {{ $sub->label }}
                                    </option>
                                @endforeach
                            </select>
